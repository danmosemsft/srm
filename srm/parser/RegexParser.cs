...
using static Microsoft.SRM.RegexParseError;
using static Microsoft.SRM.RegexParseException;
...
namespace System.Text.RegularExpressions
{
    ...
    internal ref struct RegexParser
    {
            ...
            while (CharsRight() > 0)
            {
                ...
                switch (ch)
                {
                    ...
                    case ')':
                        if (EmptyStack())
                        {
                            throw MakeException(Microsoft.SRM.RegexParseError.InsufficientOpeningParentheses, SR.InsufficientOpeningParentheses);
                        }
                    ...
                    case '\\':
                        if (CharsRight() == 0)
                        {
                            throw MakeException(Microsoft.SRM.RegexParseError.UnescapedEndingBackslash, SR.UnescapedEndingBackslash);
                        }
                    ...
                    case '?':
                        if (Unit() == null)
                        {
                            throw wasPrevQuantifier ?
                                MakeException(Microsoft.SRM.RegexParseError.NestedQuantifiersNotParenthesized, SR.Format(SR.NestedQuantifiersNotParenthesized, ch)) :
                                MakeException(Microsoft.SRM.RegexParseError.QuantifierAfterNothing, SR.QuantifierAfterNothing);
                        }
                        MoveLeft();
                        break;
                ...
                while (Unit() != null)
                {
                    ...
                    if (min > max)
                    {
                        throw MakeException(Microsoft.SRM.RegexParseError.ReversedQuantifierRange, SR.ReversedQuantifierRange);
                    }
        ...
        BreakOuterScan:
            ...
            if (!EmptyStack())
            {
                throw MakeException(Microsoft.SRM.RegexParseError.InsufficientClosingParentheses, SR.InsufficientClosingParentheses);
            }
        ...
        private RegexCharClass? ScanCharClass(bool caseInsensitive, bool scanOnly)
        {
                    ...
                    switch (ch = RightCharMoveRight())
                    {
                        ...
                        case 'd':
                            if (!scanOnly)
                            {
                                if (inRange)
                                {
                                    throw MakeException(Microsoft.SRM.RegexParseError.ShorthandClassInCharacterRange, SR.Format(SR.ShorthandClassInCharacterRange, ch));
                                }
                        ...
                        case '-':
                            if (!scanOnly)
                            {
                                if (inRange)
                                {
                                    if (chPrev > ch)
                                    {
                                        throw MakeException(Microsoft.SRM.RegexParseError.ReversedCharacterRange, SR.ReversedCharacterRange);
                                    }
                ...
                if (inRange)
                {
                    ...
                    if (!scanOnly)
                    {
                        if (ch == '[' && !translatedChar && !firstChar)
                        {
                            ...
                            if (CharsRight() > 0 && RightChar() != ']')
                            {
                                throw MakeException(Microsoft.SRM.RegexParseError.ExclusionGroupNotLast, SR.ExclusionGroupNotLast);
                            }
                        }
                        else
                        {
            ...
            if (!closed)
            {
                throw MakeException(Microsoft.SRM.RegexParseError.UnterminatedBracket, SR.UnterminatedBracket);
            }
                ...
                switch (ch)
                {
                        ...
                        switch (ch = RightCharMoveRight())
                        {
                            ...
                            default:
                                ...
                                if ((uint)(ch - '0') <= 9)
                                {
                                    ...
                                    {
                                        throw MakeException(Microsoft.SRM.RegexParseError.CaptureGroupNameInvalid, SR.CaptureGroupNameInvalid);
                                    }

                                    if (capnum == 0)
                                    {
                                        throw MakeException(Microsoft.SRM.RegexParseError.CaptureGroupOfZero, SR.CaptureGroupOfZero);
                                    }
                                }
                                ...
                                if ((capnum != -1 || proceed == true) && CharsRight() > 1 && RightChar() == '-')
                                {
                                    ...
                                    if ((uint)(ch - '0') <= 9)
                                    {
                                        ...
                                        {
                                            throw MakeException(Microsoft.SRM.RegexParseError.UndefinedNumberedReference, SR.Format(SR.UndefinedNumberedReference, uncapnum));
                                        }
                                    ...
                                    else if (RegexCharClass.IsWordChar(ch))
                                    {
                                        ...
                                        {
                                            throw MakeException(Microsoft.SRM.RegexParseError.UndefinedNamedReference, SR.Format(SR.UndefinedNamedReference, uncapname));
                                        }
                    ...
                    case '(':
                        ...
                        if (CharsRight() > 0)
                        {
                            ...
                            if (ch >= '0' && ch <= '9')
                            {
                                ...
                                if (CharsRight() > 0 && RightCharMoveRight() == ')')
                                {
                                    ...
                                    throw MakeException(Microsoft.SRM.RegexParseError.AlternationHasUndefinedReference, SR.Format(SR.AlternationHasUndefinedReference, capnum.ToString()));
                                }

                                throw MakeException(Microsoft.SRM.RegexParseError.AlternationHasMalformedReference, SR.Format(SR.AlternationHasMalformedReference, capnum.ToString()));
                            }
                            else if (RegexCharClass.IsWordChar(ch))
                            {
                                string capname = ScanCapname();
                        ...
                        if (charsRight >= 3 && RightChar(1) == '?')
                        {
                            ...
                            if (rightchar2 == '#')
                            {
                                throw MakeException(Microsoft.SRM.RegexParseError.AlternationHasComment, SR.AlternationHasComment);
                            }

                            // disallow named capture group (?<..>..) in the condition
                            if (rightchar2 == '\'')
                            {
                                throw MakeException(Microsoft.SRM.RegexParseError.AlternationHasNamedCapture, SR.AlternationHasNamedCapture);
                            }
        ...
        BreakRecognize:
            ...
            throw MakeException(Microsoft.SRM.RegexParseError.InvalidGroupingConstruct, SR.InvalidGroupingConstruct);
        }
            ...
            if (UseOptionX())
            {
                while (true)
                {
                    ...
                    else if (CharsRight() >= 3 && RightChar(2) == '#' && RightChar(1) == '?' && RightChar() == '(')
                    {
                        ...
                        if (CharsRight() == 0)
                        {
                            throw MakeException(Microsoft.SRM.RegexParseError.UnterminatedComment, SR.UnterminatedComment);
                        }
        ...
        private RegexNode? ScanBasicBackslash(bool scanOnly)
        {
            ...
            if (ch == 'k')
            {
                ...
                {
                    throw MakeException(Microsoft.SRM.RegexParseError.MalformedNamedReference, SR.MalformedNamedReference);
                }
            ...
            if (angled && ch >= '0' && ch <= '9')
            {
                ...
                if (CharsRight() > 0 && RightCharMoveRight() == close)
                {
                    return
                        ...
                        throw MakeException(Microsoft.SRM.RegexParseError.UndefinedNumberedReference, SR.Format(SR.UndefinedNumberedReference, capnum.ToString()));
                }
            }
            ...
            else if (angled && RegexCharClass.IsWordChar(ch))
            {
                ...
                if (CharsRight() > 0 && RightCharMoveRight() == close)
                {
                    return
                        ...
                        throw MakeException(Microsoft.SRM.RegexParseError.UndefinedNamedReference, SR.Format(SR.UndefinedNamedReference, capname));
                }
            }
            ...
            if (ch >= '0' && ch <= '9')
            {
                if (!angled && UseOptionE())
                {
                    ...
                    while (CharsRight() > 0 && (ch = RightChar()) >= '0' && ch <= '9')
                    {
                        ...
                        {
                            throw MakeException(Microsoft.SRM.RegexParseError.QuantifierOrCaptureGroupOutOfRange, SR.QuantifierOrCaptureGroupOutOfRange);
                        }
        ...
        private char ScanHex(int c)
        {
            ...
            if (c > 0)
            {
                throw MakeException(Microsoft.SRM.RegexParseError.InsufficientOrInvalidHexDigits, SR.InsufficientOrInvalidHexDigits);
            }
        ...
        private char ScanControl()
        {
            if (CharsRight() == 0)
            {
                throw MakeException(Microsoft.SRM.RegexParseError.MissingControlCharacter, SR.MissingControlCharacter);
            }
            ...
            throw MakeException(Microsoft.SRM.RegexParseError.UnrecognizedControlCharacter, SR.UnrecognizedControlCharacter);
        }
        ...
        private char ScanCharEscape()
        {
            ...
            switch (ch)
            {
                ...
                default:
                    ...
                    {
                        throw MakeException(Microsoft.SRM.RegexParseError.UnrecognizedEscape, SR.Format(SR.UnrecognizedEscape, ch));
                    }
                    return ch;
            }
        }
        ...
        private string ParseProperty()
        {
            if (CharsRight() < 3)
            {
                throw MakeException(Microsoft.SRM.RegexParseError.InvalidUnicodePropertyEscape, SR.InvalidUnicodePropertyEscape);
            }

            char ch = RightCharMoveRight();
            if (ch != '{')
            {
                throw MakeException(Microsoft.SRM.RegexParseError.MalformedUnicodePropertyEscape, SR.MalformedUnicodePropertyEscape);
            }
...
#if DEBUG
                'd' => (RegexOptions)0x80 /*RegexOptions.Debug*/,
#endif
        ...
        private void PopGroup()
        {
            ...
            if (_group.Type == RegexNode.Testgroup && _group.ChildCount() == 0)
            {
                if (_unit == null)
                {
                    throw MakeException(Microsoft.SRM.RegexParseError.AlternationHasMalformedCondition, SR.AlternationHasMalformedCondition);
                }
        ...
        private void AddGroup()
        {
            if (_group!.Type == RegexNode.Testgroup || _group.Type == RegexNode.Testref)
            {
                ...
                {
                    throw MakeException(Microsoft.SRM.RegexParseError.AlternationHasTooManyConditions, SR.AlternationHasTooManyConditions);
                }
            }
            else
            {
        ...
        private Microsoft.SRM.RegexParseException MakeException(Microsoft.SRM.RegexParseError error, string message) =>
            new Microsoft.SRM.RegexParseException(error, _currentPos, SR.Format(SR.MakeException, _pattern, _currentPos, message));
